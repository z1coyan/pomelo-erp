# 📐 tzjingtai-erp  
# **技术架构文档（Technical Architecture v1.0）**

---

## 1. 技术栈总览

### 核心技术栈
```
项目管理：Turborepo (Monorepo)
前端：Next.js 15+ (App Router) + React 19+ + TypeScript 5+
后端：Nest.js 11+ + TypeScript 5+
运行时：Node.js 24 LTS
数据库：PostgreSQL 17+
ORM：Prisma 6+
UI 组件：shadcn/ui (latest) + Radix UI (latest) + Tailwind CSS 4+
```

### Monorepo 架构
```
tzjingtai-erp/
├── apps/
│   ├── web/              # Next.js 前端应用
│   └── api/              # Nest.js 后端应用
├── packages/
│   ├── database/         # Prisma Schema + 数据库配置
│   ├── types/            # 共享 TypeScript 类型
│   ├── utils/            # 共享工具函数
│   ├── config/           # 共享配置（ESLint, TypeScript）
│   └── validators/       # 共享验证规则（Zod schemas）
├── turbo.json            # Turborepo 配置
├── package.json          # 根 package.json
└── pnpm-lock.yaml        # pnpm 锁文件
```

---

## 2. Monorepo 架构设计

### 2.1 Turborepo 配置

**技术选型理由**
- Turborepo：专为 Monorepo 设计，增量构建，远程缓存
- pnpm Workspaces：高效的包管理，节省磁盘空间

### 2.2 项目结构

```
tzjingtai-erp/
├── apps/
│   ├── web/                      # Next.js 前端应用
│   │   ├── app/
│   │   ├── components/
│   │   ├── package.json
│   │   └── tsconfig.json
│   └── api/                      # Nest.js 后端应用
│       ├── src/
│       ├── test/
│       ├── package.json
│       └── tsconfig.json
├── packages/
│   ├── database/                 # Prisma 数据库包
│   │   ├── prisma/
│   │   │   ├── schema.prisma
│   │   │   ├── migrations/
│   │   │   └── seed.ts
│   │   ├── src/
│   │   │   ├── client.ts         # Prisma Client 导出
│   │   │   └── index.ts
│   │   ├── package.json
│   │   └── tsconfig.json
│   ├── types/                    # 共享 TypeScript 类型
│   │   ├── src/
│   │   │   ├── api/              # API 类型
│   │   │   ├── domain/           # 领域模型类型
│   │   │   ├── dto/              # DTO 类型
│   │   │   └── index.ts
│   │   ├── package.json
│   │   └── tsconfig.json
│   ├── utils/                    # 共享工具函数
│   │   ├── src/
│   │   │   ├── date.ts
│   │   │   ├── currency.ts
│   │   │   ├── format.ts
│   │   │   └── index.ts
│   │   ├── package.json
│   │   └── tsconfig.json
│   ├── validators/               # 共享验证规则
│   │   ├── src/
│   │   │   ├── voucher.schema.ts
│   │   │   ├── invoice.schema.ts
│   │   │   └── index.ts
│   │   ├── package.json
│   │   └── tsconfig.json
│   └── config/                   # 共享配置
│       ├── eslint/
│       │   └── index.js
│       ├── typescript/
│       │   ├── base.json
│       │   ├── nextjs.json
│       │   └── nestjs.json
│       └── package.json
├── turbo.json                    # Turborepo 配置
├── package.json                  # 根 package.json
├── pnpm-lock.yaml                # pnpm 锁文件
├── pnpm-workspace.yaml           # pnpm workspace 配置
├── .gitignore
└── README.md
```

### 2.3 包依赖关系

```
apps/web
  ├── @repo/types
  ├── @repo/utils
  ├── @repo/validators
  └── @repo/config

apps/api
  ├── @repo/database
  ├── @repo/types
  ├── @repo/utils
  ├── @repo/validators
  └── @repo/config


packages/database
  └── @repo/types
```

### 2.4 Turborepo 配置文件

**turbo.json**
```json
{
  "$schema": "https://turbo.build/schema.json",
  "globalDependencies": ["**/.env.*local"],
  "pipeline": {
    "build": {
      "dependsOn": ["^build"],
      "outputs": [".next/**", "dist/**"]
    },
    "dev": {
      "cache": false,
      "persistent": true
    },
    "lint": {
      "dependsOn": ["^lint"]
    },
    "type-check": {
      "dependsOn": ["^type-check"]
    },
    "test": {
      "dependsOn": ["^build"]
    },
    "db:generate": {
      "cache": false
    },
    "db:push": {
      "cache": false
    },
    "db:migrate": {
      "cache": false
    }
  }
}
```

**根 package.json**
```json
{
  "name": "tzjingtai-erp",
  "version": "1.0.0",
  "private": true,
  "engines": {
    "node": ">=24.0.0",
    "pnpm": ">=9.0.0"
  },
  "packageManager": "pnpm@9.0.0",
  "scripts": {
    "dev": "turbo run dev",
    "build": "turbo run build",
    "lint": "turbo run lint",
    "type-check": "turbo run type-check",
    "test": "turbo run test",
    "db:generate": "turbo run db:generate",
    "db:push": "turbo run db:push",
    "db:migrate": "turbo run db:migrate",
    "clean": "turbo run clean && rm -rf node_modules"
  },
  "devDependencies": {
    "turbo": "^2.0.0",
    "@repo/config": "workspace:*",
    "typescript": "^5.6.0"
  }
}
```

**pnpm-workspace.yaml**
```yaml
packages:
  - 'apps/*'
  - 'packages/*'
```

### 2.5 共享包说明


#### @repo/database
- Prisma Schema
- Prisma Client 导出
- 数据库迁移
- Seed 脚本

#### @repo/types
- API 请求/响应类型
- 领域模型类型
- Enum 定义
- 前后端共享类型

#### @repo/utils
- 日期格式化
- 货币计算
- 字符串处理
- 通用工具函数

#### @repo/validators
- Zod Schema 定义
- 前后端共享验证规则
- 自定义验证器

#### @repo/config
- ESLint 配置
- TypeScript 配置
- Prettier 配置

### 2.6 Monorepo 优势

- **代码复用**：类型、工具函数统一管理；UI 组件位于 apps/web
- **类型安全**：前后端共享类型定义，避免不一致
- **统一构建**：Turborepo 增量构建，提升效率
- **依赖管理**：统一版本管理，避免冲突
- **开发体验**：一次启动，前后端同时开发

---

## 3. 系统架构图

```
┌─────────────────────────────────────────────────────────────┐
│                         前端层 (Next.js)                      │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │  Web UI      │  │  API Routes  │  │  SSR/SSG     │      │
│  │ (shadcn/ui)  │  │  (BFF层)     │  │              │      │
│  └──────────────┘  └──────────────┘  └──────────────┘      │
└─────────────────────────────────────────────────────────────┘
                            ↓ HTTP/REST
┌─────────────────────────────────────────────────────────────┐
│                      后端层 (Nest.js)                         │
│  ┌──────────────────────────────────────────────────────┐   │
│  │              Controller Layer (API)                   │   │
│  └──────────────────────────────────────────────────────┘   │
│  ┌──────────────────────────────────────────────────────┐   │
│  │              Service Layer (业务逻辑)                  │   │
│  └──────────────────────────────────────────────────────┘   │
│  ┌──────────────────────────────────────────────────────┐   │
│  │              Domain Layer (领域模型)                   │   │
│  │  • Event Engine  • PostingRule Engine                │   │
│  │  • Voucher Engine  • Permission Engine               │   │
│  └──────────────────────────────────────────────────────┘   │
│  ┌──────────────────────────────────────────────────────┐   │
│  │              Repository Layer (数据访问)               │   │
│  └──────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
                            ↓ Prisma ORM
┌─────────────────────────────────────────────────────────────┐
│                   数据层 (PostgreSQL)                         │
│  • 多公司隔离  • 多账簿隔离  • 事务一致性                      │
└─────────────────────────────────────────────────────────────┘
```

---

## 4. 前端架构

### 4.1 技术选型

| 技术 | 版本 | 用途 |
|------|------|------|
| Next.js | 15+ | React 框架（App Router） |
| React | 19+ | UI 框架 |
| TypeScript | 5+ | 类型安全 |
| shadcn/ui | latest | UI 组件库 |
| Radix UI | latest | 无样式组件基础 |
| Tailwind CSS | 4+ | 样式框架 |
| Zustand | 5+ | 状态管理 |
| React Hook Form | 7+ | 表单管理 |
| Zod | 3+ | 表单验证 |
| TanStack Query | 5+ | 数据获取/缓存 |
| TanStack Table | 8+ | 表格组件 |
| date-fns | 4+ | 日期处理 |
| Recharts | 2+ | 图表库 |

### 4.2 目录结构（apps/web）

```
apps/web/
├── app/                          # Next.js App Router
│   ├── (auth)/                   # 认证路由组
│   │   ├── login/
│   │   └── layout.tsx
│   ├── (dashboard)/              # 主应用路由组
│   │   ├── finance/              # 财务模块
│   │   │   ├── vouchers/
│   │   │   ├── gl-entries/
│   │   │   └── chart-of-accounts/
│   │   ├── business/             # 业务模块
│   │   │   ├── accept-bills/
│   │   │   ├── vat-invoices/
│   │   │   └── payments/
│   │   └── layout.tsx
│   ├── api/                      # API Routes (BFF)
│   └── layout.tsx
├── components/                   # 页面级组件
│   ├── forms/                    # 表单组件
│   ├── tables/                   # 表格组件
│   └── layouts/                  # 布局组件
├── lib/                          # 工具库
│   ├── api-client.ts             # API 客户端
│   └── utils.ts
├── hooks/                        # 自定义 Hooks
├── stores/                       # Zustand stores
├── public/                       # 静态资源
├── package.json
└── tsconfig.json
```

### 4.3 UI 组件规范（apps/web/components）

**基础组件（shadcn/ui）**
- Button, Input, Select, Checkbox, Radio
- Dialog, Sheet, Popover, Dropdown Menu
- Table, Card, Tabs, Badge, Alert
- Form, Label, Textarea
- Calendar, DatePicker
- Toast, Tooltip

**业务组件**
- VoucherForm（凭证录入表单）
- VoucherTable（凭证列表）
- AccountSelector（科目选择器）
- AuxiliarySelector（辅助核算选择器）
- CurrencyInput（货币输入框）
- AttachmentUploader（附件上传）
- AuditLogViewer（审计日志查看器）

### 4.4 前端特性

- **快捷键支持**：财务录入场景（Ctrl+S 保存、Ctrl+Enter 提交等）
- **批量编辑**：凭证分录批量录入
- **实时验证**：借贷平衡校验
- **离线缓存**：TanStack Query 缓存策略
- **权限控制**：基于角色的 UI 显示/隐藏

---

## 5. 后端架构

### 5.1 技术选型

| 技术 | 版本 | 用途 |
|------|------|------|
| Node.js | 24 LTS | JavaScript 运行时 |
| Nest.js | 11+ | Node.js 框架 |
| TypeScript | 5+ | 类型安全 |
| Prisma | 6+ | ORM |
| PostgreSQL | 17+ | 关系型数据库 |
| class-validator | 0.14+ | DTO 验证 |
| class-transformer | 0.5+ | 对象转换 |
| Passport.js | 0.7+ | 认证中间件 |
| @nestjs/jwt | 10+ | JWT 认证 |
| bcrypt | 5+ | 密码加密 |
| Winston | 3+ | 日志框架 |
| Bull | 4+ | 任务队列（可选） |
| ali-oss | 6+ | 阿里云 OSS SDK |
| @alicloud/ocr | latest | 阿里云 OCR SDK |

### 5.2 目录结构（apps/api）

```
apps/api/
├── src/
│   ├── main.ts                   # 应用入口
│   ├── app.module.ts             # 根模块
│   ├── common/                   # 公共模块
│   │   ├── decorators/           # 装饰器
│   │   ├── filters/              # 异常过滤器
│   │   ├── guards/               # 守卫（权限）
│   │   ├── interceptors/         # 拦截器
│   │   └── pipes/                # 管道（验证）
│   ├── config/                   # 配置
│   │   ├── database.config.ts
│   │   ├── jwt.config.ts
│   │   └── logger.config.ts
│   ├── modules/                  # 业务模块
│   │   ├── auth/                 # 认证模块
│   │   ├── users/                # 用户模块
│   │   ├── companies/            # 公司模块
│   │   ├── ledgers/              # 账簿模块
│   │   ├── chart-of-accounts/    # 科目模块
│   │   ├── vouchers/             # 凭证模块
│   │   ├── accounting-events/    # 会计事件模块
│   │   ├── posting-rules/        # 过账规则模块
│   │   ├── accept-bills/         # 承兑汇票模块
│   │   ├── vat-invoices/         # 增值税发票模块
│   │   ├── payments/             # 资金模块
│   │   ├── inventory/            # 库存模块
│   │   ├── attachments/          # 附件模块
│   │   └── ocr/                  # OCR 识别模块
│   └── domain/                   # 领域层
│       ├── engines/              # 核心引擎
│       │   ├── event.engine.ts
│       │   ├── posting-rule.engine.ts
│       │   └── voucher.engine.ts
│       ├── entities/             # 领域实体
│       └── value-objects/        # 值对象
├── test/                         # 测试
├── package.json
└── tsconfig.json
```

### 5.3 模块结构（标准）

每个业务模块遵循以下结构：

```
module-name/
├── dto/                          # 数据传输对象
│   ├── create-xxx.dto.ts
│   ├── update-xxx.dto.ts
│   └── query-xxx.dto.ts
├── entities/                     # 实体（可选）
│   └── xxx.entity.ts
├── xxx.controller.ts             # 控制器
├── xxx.service.ts                # 服务
├── xxx.repository.ts             # 仓储（可选）
└── xxx.module.ts                 # 模块定义
```

---

## 6. 核心引擎设计

### 6.1 会计事件引擎（Event Engine）

**职责**
- 接收业务动作触发
- 创建 AccountingEvent 记录
- 匹配 PostingRule
- 调用 Voucher Engine 生成凭证草稿

**核心方法**
```typescript
class EventEngine {
  async createEvent(params: CreateEventParams): Promise<AccountingEvent>
  async processEvent(eventId: string): Promise<Voucher>
  async reverseEvent(eventId: string): Promise<Voucher>
}
```

### 6.2 过账规则引擎（PostingRule Engine）

**职责**
- 根据 eventType + ledgerId 匹配规则
- 解析 amountFormula
- 生成凭证分录（VoucherItem）

**核心方法**
```typescript
class PostingRuleEngine {
  async matchRules(eventType: string, ledgerId: string): Promise<PostingRule[]>
  async generateVoucherItems(event: AccountingEvent, rules: PostingRule[]): Promise<VoucherItem[]>
}
```

### 6.3 凭证引擎（Voucher Engine）

**职责**
- 创建凭证草稿
- 验证借贷平衡
- 过账（Post）
- 红冲（Reverse）
- 写入总账（GL Entry）

**核心方法**
```typescript
class VoucherEngine {
  async createDraft(items: VoucherItem[]): Promise<Voucher>
  async validateBalance(voucher: Voucher): Promise<boolean>
  async post(voucherId: string): Promise<void>
  async reverse(voucherId: string): Promise<Voucher>
}
```

---

## 7. 数据库设计（packages/database）

### 7.1 PostgreSQL 特性使用

- **事务隔离级别**：READ COMMITTED（默认）
- **外键约束**：强制引用完整性
- **索引策略**：
  - 主键自动索引
  - 外键字段索引
  - 查询高频字段（companyId, ledgerId, status）
  - 复合索引（eventType + ledgerId）
- **分区表**（可选）：GLEntry 按年份分区
- **软删除**：deletedAt 字段

### 7.2 Prisma Schema 规范

```prisma
// 示例：Voucher 模型
model Voucher {
  id              String         @id @default(uuid())
  voucherNo       String         @unique
  ledgerId        String
  ledger          Ledger         @relation(fields: [ledgerId], references: [id])
  voucherDate     DateTime
  status          VoucherStatus  @default(DRAFT)
  totalDebit      Decimal        @db.Decimal(18, 2)
  totalCredit     Decimal        @db.Decimal(18, 2)
  eventId         String?        @unique
  event           AccountingEvent? @relation(fields: [eventId], references: [id])
  items           VoucherItem[]
  createdBy       String
  createdAt       DateTime       @default(now())
  updatedAt       DateTime       @updatedAt
  deletedAt       DateTime?
  
  @@index([ledgerId, voucherDate])
  @@index([status])
}
```

---

### 7.3 数据库包结构

```
packages/database/
├── prisma/
│   ├── schema.prisma             # Prisma Schema
│   ├── migrations/               # 数据库迁移
│   └── seed.ts                   # 初始数据
├── src/
│   ├── client.ts                 # Prisma Client 导出
│   └── index.ts
├── package.json
└── tsconfig.json
```

**package.json**
```json
{
  "name": "@repo/database",
  "version": "0.0.0",
  "private": true,
  "main": "./src/index.ts",
  "scripts": {
    "db:generate": "prisma generate",
    "db:push": "prisma db push",
    "db:migrate": "prisma migrate dev",
    "db:seed": "tsx prisma/seed.ts"
  },
  "dependencies": {
    "@prisma/client": "^6.0.0"
  },
  "devDependencies": {
    "prisma": "^6.0.0",
    "@repo/types": "workspace:*"
  }
}
```

---

## 8. 文件存储与 OCR 服务

### 8.1 阿里云 OSS 集成

**技术选型**
- **ali-oss**：阿里云官方 Node.js SDK
- **存储桶策略**：私有读写，通过签名 URL 访问

**功能特性**
- 附件上传（凭证、发票、合同等）
- 文件下载（签名 URL，有效期可配置）
- 文件删除（软删除 + 定期清理）
- 缩略图生成（图片类附件）
- 分片上传（大文件支持）

**目录结构**
```
OSS Bucket: tzjingtai-erp-files
├── attachments/
│   ├── vouchers/           # 凭证附件
│   │   └── {companyId}/{year}/{month}/{voucherId}/{filename}
│   ├── invoices/           # 发票附件
│   │   └── {companyId}/{year}/{month}/{invoiceId}/{filename}
│   ├── accept-bills/       # 承兑汇票附件
│   │   └── {companyId}/{year}/{month}/{billId}/{filename}
│   └── contracts/          # 合同附件
│       └── {companyId}/{year}/{contractId}/{filename}
└── temp/                   # 临时文件（24小时自动清理）
```

**配置示例**
```typescript
// apps/api/src/config/oss.config.ts
export const ossConfig = {
  region: process.env.OSS_REGION,
  accessKeyId: process.env.OSS_ACCESS_KEY_ID,
  accessKeySecret: process.env.OSS_ACCESS_KEY_SECRET,
  bucket: process.env.OSS_BUCKET,
  secure: true,
  timeout: 60000,
};
```

**核心方法**
```typescript
// apps/api/src/modules/attachments/oss.service.ts
class OssService {
  async upload(file: Buffer, path: string): Promise<string>
  async getSignedUrl(path: string, expiresIn: number): Promise<string>
  async delete(path: string): Promise<void>
  async exists(path: string): Promise<boolean>
  async generateThumbnail(imagePath: string): Promise<string>
}
```

### 8.2 阿里云 OCR 集成

**技术选型**
- **@alicloud/ocr**：阿里云 OCR SDK
- **支持类型**：增值税发票、银行承兑汇票、身份证、营业执照等

**OCR 识别场景**

| 场景 | OCR 类型 | 识别字段 |
|------|----------|----------|
| 增值税专用发票 | VATInvoice | 发票代码、发票号码、开票日期、金额、税额、购销方信息 |
| 增值税普通发票 | VATInvoice | 同上 |
| 银行承兑汇票 | BankAcceptance | 票据号码、出票日期、到期日、金额、出票人、收款人 |
| 银行回单 | BankReceipt | 交易日期、金额、对方账户、摘要 |
| 身份证 | IDCard | 姓名、身份证号、地址（可选） |
| 营业执照 | BusinessLicense | 公司名称、统一社会信用代码、法人、地址 |

**配置示例**
```typescript
// apps/api/src/config/ocr.config.ts
export const ocrConfig = {
  accessKeyId: process.env.OCR_ACCESS_KEY_ID,
  accessKeySecret: process.env.OCR_ACCESS_KEY_SECRET,
  endpoint: 'https://ocr.cn-shanghai.aliyuncs.com',
};
```

**核心方法**
```typescript
// apps/api/src/modules/ocr/ocr.service.ts
class OcrService {
  async recognizeVATInvoice(imageUrl: string): Promise<VATInvoiceOcrResult>
  async recognizeBankAcceptance(imageUrl: string): Promise<BankAcceptanceOcrResult>
  async recognizeBankReceipt(imageUrl: string): Promise<BankReceiptOcrResult>
  async recognizeIDCard(imageUrl: string): Promise<IDCardOcrResult>
  async recognizeBusinessLicense(imageUrl: string): Promise<BusinessLicenseOcrResult>
}
```

**OCR 结果类型**
```typescript
// packages/types/src/ocr/vat-invoice.ts
export interface VATInvoiceOcrResult {
  invoiceCode: string;           // 发票代码
  invoiceNumber: string;          // 发票号码
  invoiceDate: string;            // 开票日期
  invoiceType: string;            // 发票类型（专票/普票）
  totalAmount: number;            // 价税合计
  totalTax: number;               // 税额
  totalWithoutTax: number;        // 不含税金额
  purchaserName: string;          // 购方名称
  purchaserTaxNumber: string;     // 购方税号
  sellerName: string;             // 销方名称
  sellerTaxNumber: string;        // 销方税号
  items: Array<{                  // 明细行
    name: string;
    specification: string;
    unit: string;
    quantity: number;
    unitPrice: number;
    amount: number;
    taxRate: number;
    tax: number;
  }>;
  confidence: number;             // 识别置信度
}
```

### 8.3 附件模块设计

**数据模型**
```prisma
// packages/database/prisma/schema.prisma
model Attachment {
  id              String         @id @default(uuid())
  companyId       String
  company         Company        @relation(fields: [companyId], references: [id])
  entityType      EntityType     // VOUCHER, INVOICE, ACCEPT_BILL, CONTRACT
  entityId        String         // 关联业务单据 ID
  fileName        String         // 原始文件名
  fileSize        Int            // 文件大小（字节）
  mimeType        String         // MIME 类型
  ossPath         String         // OSS 存储路径
  thumbnailPath   String?        // 缩略图路径（图片）
  ocrStatus       OcrStatus      @default(PENDING) // PENDING, PROCESSING, SUCCESS, FAILED
  ocrResult       Json?          // OCR 识别结果（JSON）
  ocrConfidence   Float?         // OCR 置信度
  ocrErrorMsg     String?        // OCR 错误信息
  uploadedBy      String
  uploadedAt      DateTime       @default(now())
  deletedAt       DateTime?
  
  @@index([companyId, entityType, entityId])
  @@index([ocrStatus])
}

enum EntityType {
  VOUCHER
  INVOICE
  ACCEPT_BILL
  CONTRACT
  BANK_RECEIPT
  ID_CARD
  BUSINESS_LICENSE
}

enum OcrStatus {
  PENDING       // 待识别
  PROCESSING    // 识别中
  SUCCESS       // 识别成功
  FAILED        // 识别失败
  SKIPPED       // 跳过识别（非图片文件）
}
```

**业务流程**
```
1. 用户上传文件
   ↓
2. 后端接收文件 → 上传到 OSS
   ↓
3. 保存 Attachment 记录（ocrStatus: PENDING）
   ↓
4. 异步触发 OCR 识别（Bull 队列）
   ↓
5. OCR 识别完成 → 更新 ocrResult
   ↓
6. 前端展示识别结果 → 用户确认/修改
   ↓
7. 用户保存 → 创建业务单据（发票/承兑等）
```

**API 接口**
```typescript
// 上传附件
POST /api/v1/attachments/upload
Content-Type: multipart/form-data
Body: {
  file: File,
  entityType: 'INVOICE',
  entityId: 'xxx' (可选)
}

// 获取附件列表
GET /api/v1/attachments?entityType=INVOICE&entityId=xxx

// 获取附件详情（含 OCR 结果）
GET /api/v1/attachments/:id

// 获取附件下载链接（签名 URL）
GET /api/v1/attachments/:id/download

// 触发 OCR 识别
POST /api/v1/attachments/:id/ocr

// 删除附件
DELETE /api/v1/attachments/:id
```

### 8.4 前端附件组件

**AttachmentUploader 组件**
```typescript
// apps/web/components/attachment-uploader.tsx
interface AttachmentUploaderProps {
  entityType: EntityType;
  entityId?: string;
  maxFiles?: number;
  maxSize?: number;
  accept?: string;
  enableOcr?: boolean;
  onUploadSuccess?: (attachment: Attachment) => void;
  onOcrComplete?: (result: OcrResult) => void;
}
```

**功能特性**
- 拖拽上传
- 多文件上传
- 上传进度显示
- 文件预览（图片/PDF）
- OCR 识别状态显示
- OCR 结果展示与编辑

### 8.5 环境变量配置

```env
# 阿里云 OSS
OSS_REGION="oss-cn-shanghai"
OSS_ACCESS_KEY_ID="your-access-key-id"
OSS_ACCESS_KEY_SECRET="your-access-key-secret"
OSS_BUCKET="tzjingtai-erp-files"
OSS_ENDPOINT="https://oss-cn-shanghai.aliyuncs.com"

# 阿里云 OCR
OCR_ACCESS_KEY_ID="your-access-key-id"
OCR_ACCESS_KEY_SECRET="your-access-key-secret"
OCR_ENDPOINT="https://ocr.cn-shanghai.aliyuncs.com"

# 文件上传限制
MAX_FILE_SIZE=10485760  # 10MB
ALLOWED_FILE_TYPES="image/jpeg,image/png,image/jpg,application/pdf"
```

### 8.6 安全策略

- **访问控制**：所有文件通过签名 URL 访问，有效期 1 小时
- **权限校验**：下载前验证用户是否有权限访问该附件
- **文件扫描**：上传时进行病毒扫描（可选）
- **敏感信息**：OCR 结果中的敏感信息脱敏存储
- **审计日志**：记录所有文件上传、下载、删除操作

### 8.7 成本优化

- **存储类型**：标准存储（热数据）+ 归档存储（历史数据）
- **生命周期规则**：
  - temp/ 目录文件 24 小时自动删除
  - 3 年以上附件转归档存储
- **CDN 加速**：高频访问文件使用 CDN（可选）
- **压缩策略**：图片自动压缩（OSS 图片处理）

---

## 9. 权限控制

### 9.1 RBAC 模型

```
User → UserRole → Role → RolePermission → Permission
```

### 9.2 权限粒度

- **模块级**：finance, business, system
- **操作级**：create, read, update, delete, approve, post
- **数据级**：
  - 公司隔离（companyId）
  - 账簿隔离（ledgerId）
  - 状态隔离（Draft 可改，Posted 不可改）

### 9.3 实现方式

**后端守卫（Guard）**
```typescript
@UseGuards(JwtAuthGuard, PermissionGuard)
@RequirePermissions('voucher:create')
@Controller('vouchers')
export class VoucherController {}
```

**前端权限 Hook**
```typescript
const { hasPermission } = usePermissions();
if (hasPermission('voucher:approve')) {
  // 显示审核按钮
}
```

---

## 10. 日志系统

### 10.1 日志框架：Winston

**日志级别**
- error：错误日志
- warn：警告日志
- info：信息日志
- debug：调试日志

**日志输出**
- 控制台（开发环境）
- 文件（生产环境）
  - `logs/error.log`
  - `logs/combined.log`
- 数据库（审计日志）

### 10.2 审计日志（AuditLog）

**记录内容**
- 用户操作（userId, username）
- 操作类型（action: CREATE, UPDATE, DELETE, APPROVE, POST）
- 操作对象（entityType, entityId）
- 操作前后数据（oldValue, newValue）
- IP 地址、时间戳

**实现方式**
```typescript
@UseInterceptors(AuditLogInterceptor)
@Post()
async create(@Body() dto: CreateVoucherDto) {}
```

---

## 11. API 设计规范

### 11.1 RESTful API

**URL 规范**
```
GET    /api/v1/vouchers          # 列表
GET    /api/v1/vouchers/:id      # 详情
POST   /api/v1/vouchers          # 创建
PATCH  /api/v1/vouchers/:id      # 更新
DELETE /api/v1/vouchers/:id      # 删除
POST   /api/v1/vouchers/:id/post # 过账（动作）
```

### 11.2 响应格式

**成功响应**
```json
{
  "success": true,
  "data": { ... },
  "message": "操作成功"
}
```

**错误响应**
```json
{
  "success": false,
  "error": {
    "code": "VOUCHER_NOT_BALANCED",
    "message": "凭证借贷不平衡",
    "details": { ... }
  }
}
```

### 11.3 分页规范

**请求参数**
```
GET /api/v1/vouchers?page=1&pageSize=20&sortBy=voucherDate&order=desc
```

**响应格式**
```json
{
  "success": true,
  "data": {
    "items": [...],
    "total": 100,
    "page": 1,
    "pageSize": 20,
    "totalPages": 5
  }
}
```

---

## 12. 安全策略

### 12.1 认证方式

- **JWT Token**：有效期 24 小时
- **Refresh Token**：有效期 7 天
- **密码加密**：bcrypt（salt rounds: 10）

### 12.2 安全措施

- CORS 配置（白名单）
- Helmet.js（HTTP 头安全）
- Rate Limiting（API 限流）
- SQL 注入防护（Prisma 参数化查询）
- XSS 防护（输入验证 + 输出转义）
- CSRF 防护（SameSite Cookie）

---

## 13. 性能优化

### 13.1 数据库优化

- 索引优化
- 查询优化（避免 N+1）
- 连接池配置
- 分页查询
- 缓存策略（Redis，可选）

### 13.2 前端优化

- Next.js SSR/SSG
- 代码分割（动态导入）
- 图片优化（next/image）
- TanStack Query 缓存
- 虚拟滚动（大列表）

---

## 14. 部署架构

### 14.1 开发环境

```
Node.js 24 LTS (本地运行)
PostgreSQL 17 (Docker)
pnpm 9+ (包管理器)
```

### 14.2 生产环境（推荐）

**注意**：生产环境文件存储使用阿里云 OSS

```
前端：Vercel / 自建服务器（Nginx）
后端：Docker + PM2 / Kubernetes
数据库：PostgreSQL（云服务 / 自建）
文件存储：阿里云 OSS
OCR 服务：阿里云 OCR
```

---

## 15. 开发规范

### 15.1 代码规范

- **ESLint**：代码检查
- **Prettier**：代码格式化
- **Husky**：Git Hooks
- **Commitlint**：提交信息规范

### 15.2 Git 工作流

- **分支策略**：Git Flow
  - main：生产分支
  - develop：开发分支
  - feature/*：功能分支
  - hotfix/*：修复分支

### 15.3 测试策略

- **单元测试**：Jest
- **集成测试**：Supertest
- **E2E 测试**：Playwright（可选）

---

## 16. 技术债务管理

### 16.1 待优化项

- [ ] Redis 缓存层
- [ ] 消息队列（Bull）
- [ ] 微服务拆分（可选）
- [ ] GraphQL API（可选）
- [ ] 实时通知（WebSocket）

### 16.2 监控告警

- 应用监控（可选：Sentry）
- 性能监控（可选：New Relic）
- 日志聚合（可选：ELK）

---

## 17. 技术选型理由

| 技术 | 理由 |
|------|------|
| 技术 | 理由 |
|------|------|
| Node.js 24 LTS | 长期支持版本，性能优异，生态成熟 |
| Turborepo | Monorepo 专用，增量构建，远程缓存 |
| pnpm | 快速、节省磁盘空间、严格的依赖管理 |
| Next.js 15 | 最新 App Router，React 19 支持，性能优化 |
| React 19 | 最新特性，性能提升，Server Components |
| Nest.js 11 | 企业级框架，模块化，依赖注入，最新特性 |
| Prisma 6 | 类型安全，迁移管理，性能优化，最新特性 |
| PostgreSQL 17 | 最新稳定版，性能提升，新特性支持 |
| Tailwind CSS 4 | 最新版本，性能优化，新特性 |
| shadcn/ui | 可定制，无依赖锁定，代码可控，持续更新 |
| Winston | 灵活，多输出，生产级 |
| 阿里云 OSS | 稳定可靠，成本可控，国内访问速度快 |
| 阿里云 OCR | 识别准确率高，支持多种票据类型，按量付费 |

---

## 18. 附录

### 18.1 环境变量配置

```env
# 数据库
DATABASE_URL="postgresql://user:password@localhost:5432/tzjingtai_erp"

# JWT
JWT_SECRET="your-secret-key"
JWT_EXPIRES_IN="24h"

# 应用
NODE_ENV="development"
PORT=3000
API_PREFIX="/api/v1"

# 阿里云 OSS
OSS_REGION="oss-cn-shanghai"
OSS_ACCESS_KEY_ID="your-access-key-id"
OSS_ACCESS_KEY_SECRET="your-access-key-secret"
OSS_BUCKET="tzjingtai-erp-files"
OSS_ENDPOINT="https://oss-cn-shanghai.aliyuncs.com"

# 阿里云 OCR
OCR_ACCESS_KEY_ID="your-access-key-id"
OCR_ACCESS_KEY_SECRET="your-access-key-secret"
OCR_ENDPOINT="https://ocr.cn-shanghai.aliyuncs.com"

# 文件上传限制
MAX_FILE_SIZE=10485760  # 10MB
ALLOWED_FILE_TYPES="image/jpeg,image/png,image/jpg,application/pdf"
```

### 18.2 启动命令（Monorepo）

```bash
# 安装 pnpm（如果未安装）
npm install -g pnpm

# 安装依赖
pnpm install

# 生成 Prisma Client
pnpm db:generate

# 数据库迁移
pnpm db:migrate

# 启动开发环境（前后端同时启动）
pnpm dev

# 单独启动前端
cd apps/web
pnpm dev

# 单独启动后端
cd apps/api
pnpm start:dev

# 构建所有应用
pnpm build

# 类型检查
pnpm type-check

# 代码检查
pnpm lint
```



---

**文档版本**：v1.0  
**最后更新**：2025-11-14  
**维护者**：开发团队
